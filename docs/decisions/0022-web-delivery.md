...
- Fill in `index.html`: title, description, Open Graph and Twitter tags, theme colour, and a real
  `manifest.json` with icons, so the app is installable. Chromium's installability contract is:
  served over HTTPS, `manifest.json` present with `name` or `short_name`, `start_url`, and
  `display`, plus 192×192 and 512×512 icons declared in the manifest and actually present in the
  build output. A service worker is not required for installability — Flutter 3.47.4 does not
  generate one by default — so a custom service worker is only added, and documented, if offline
  behaviour is actually needed. A CI or manual check against a deployed build verifies the
...
- No service worker ships unless offline support is explicitly required; installability alone does
  not need one.
