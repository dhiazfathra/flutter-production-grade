...
localization keys rather than literal strings. `TextFormField.validator` displays whatever string
it is given, so a returned key is never wired directly to it: each field wraps its validator with a
presentation-layer adapter — `(key) => key == null ? null : context.l10n.forValidationKey(key)` —
that translates the key into the localized message before assignment. Validators themselves stay
pure and return `null` for valid input. View models own submission state; the widget owns only the
`GlobalKey<FormState>`.
